...
void applyclipping(char* buffer, int r)
{
	short* samples = (short*) buffer;
	int i;

	// short clip_thresh = 0.5 * 32768;
	short clip_thresh = 4096;
	short clip_scaling = 1;
	double data = 0;
	...
	for (i=0;i<r;i++)
	{
		...
		for (chn=0;chn<channels;chn++)
		{
			// samples[i*channels+chn] = ;

			// samples[i*channels+chn] =  (abs(samples[i*channels+chn]) < clip_thresh) ? samples[i*channels+chn] : (sign(samples[i*channels+chn]) * clip_thresh * clip_scaling);
			data = (float) samples[i * channels + chn];
			data = data * data / 32768;
			samples[i * channels + chn] = (short) data;
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	printf("Format width in bits %d", snd_pcm_format_width(format));
